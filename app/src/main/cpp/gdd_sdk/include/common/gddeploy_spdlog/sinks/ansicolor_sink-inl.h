...
{
    std::lock_guard<mutex_t> lock(mutex_);
    formatter_ = std::unique_ptr<gddeploy_spdlog::formatter>(new pattern_formatter(pattern));
}

template<typename ConsoleMutex>
SPDLOG_INLINE void ansicolor_sink<ConsoleMutex>::set_formatter(std::unique_ptr<gddeploy_spdlog::formatter> sink_formatter)
{
    std::lock_guard<mutex_t> lock(mutex_);
