...
// logger->info("Some buffer {}", gddeploy_spdlog::to_hex(v));
// char buf[128];
// logger->info("Some buffer {:X}", gddeploy_spdlog::to_hex(std::begin(buf), std::end(buf)));
// logger->info("Some buffer {:X}", gddeploy_spdlog::to_hex(std::begin(buf), std::end(buf), 16));
...
template<typename T>
struct formatter<gddeploy_spdlog::details::dump_info<T>>
{
    ...
    auto format(const gddeploy_spdlog::details::dump_info<Container> &the_range, FormatContext &ctx) -> decltype(ctx.out())
    {
