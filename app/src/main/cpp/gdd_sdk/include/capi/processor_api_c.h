...
int Infer(void *handle, const char *input_data, int input_size, int ori_img_w, int ori_img_h, int img_w, int img_h,  DetectResult *output_result);

int InitJna(void **handle, const char *config, const char *model_path, const char *license_file);
int InferJna(void *handle, const char *input_data, int input_size,BufSurfaceColorFormat format, int img_w, int img_h,  DetectResult *output_result);
#ifdef __cplusplus
}
#endif
