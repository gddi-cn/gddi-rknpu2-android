...
void CommandParser::ParseArgs(int argc, const char **argv)
{
    ...
    for (int i = 1; i < argc; ++i) {
        if (i + 1 < argc && argv[i][0] == '-' && argv[i + 1][0] != '-') {
            // Skip the next argument, since it's a value associated with the current flag
            i++; // Increment here to skip the associated value
            continue;
        }
        // If we reach this point, it means the argument format is not correct
        ShowUsage();
    }
    for (int i = 1; i < argc; i += 2) { // Increment by 2 to jump over the value associated with each command
        if (commands_.find(argv[i]) == commands_.end()) {
            ShowUsage();
        }
        // No need to increment i here, as it's being incremented in the loop header
    }
    for (int i = 1; i < argc; ++i) {
        if (argv[i][0] == '-') {
